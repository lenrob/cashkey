...
| Quarantined `any` | PR 0.0 scoped `no-explicit-any` off for `SankeyDiagram.tsx` — 19 sites, all d3-sankey layout objects. `src/components/sankey/**` was part of the original grant but PR 0.3 found it dead (nothing imported it) and deleted it, so the quarantine now covers this one file. The rule stays an error everywhere else. **The exemption covers existing code only, not new work.** New functions and components in this file must be typed — PR 1.5 adds a fourth Sankey column, so this lands during the phase where typing matters most. Where d3-sankey's types genuinely fight back, use a line-level `eslint-disable-next-line` rather than leaning on the file-level exemption. |
